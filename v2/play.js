...
(function () {
  ...
  function showSvLoading(msg, sub, opts) {
    ...
    if (t) t.textContent = msg || st('txtSvLoading', '載入街景中…');
    if (s) s.textContent = sub != null ? sub : st('txtSvLoadingSub', '首次載入可能需要幾秒');
  ...
  function updateActionButtons() {
    ...
    var txtQuiz = st('txtQuiz', '開始任務');
    var txtDone = st('txtDone', '已完成');
  ...
  function showEnding() {
    ...
    var titleTpl = st('endingTitle', '恭喜集齊 {n} 個印！').replace('{n}', String(got));
    var desc = st('endingDesc', '你已經完成探索！明信片顯示你全對的景點印章。');
    if (isOneShot() && got < n) {
      desc = st('endingDescPartial', '你已完成所有站點。全對的景點已蓋印（{c} / {n}）。').replace('{c}', String(got)).replace('{n}', String(n));
    }
    var pTitle = st('postcardTitle', st('landingTitle', '地理明信片'));
    var replay = st('txtReplay', '再玩一次');
  ...
  async function loadProgram() {
    ...
    var startBtns = document.querySelectorAll('#landing button');
    startBtns.forEach(function (b) {
      if ((b.getAttribute('onclick') || '').indexOf('startGame') >= 0) {
        b.innerHTML = st('txtStart', '開始冒險') + ' <i class="fas fa-arrow-right ml-2"></i>';
      }
    });
  ...
  function applyChromeTexts() {
    ...
    if (prev) prev.innerHTML = '<i class="fas fa-chevron-left"></i> ' + st('txtPrev', '上一站');
    if (next) next.innerHTML = st('txtNext', '下一站') + ' <i class="fas fa-chevron-right"></i>';
  }
  ...
  function renderIntro() {
    ...
    var bp = document.getElementById('btn-intro-prev');
    var bn = document.getElementById('btn-intro-next');
    var be = document.getElementById('btn-intro-enter');
    if (bp) bp.textContent = st('txtIntroPrev', '前');
    if (bn) bn.textContent = st('txtIntroNext', '後');
    if (be) be.textContent = st('txtIntroEnter', '進入景點');
  }
  ...
  window.loadLocation = function (i) {
    ...
    var fmt = st('progressFormat', '第 {i} / {total} 站').replace('{i}', String(i + 1)).replace('{total}', String(locations.length));
    ...
    showSvLoading(st('txtSvLoading', '載入街景中…'), loc.name);
    ensurePano();
    ...
    svService.getPanorama({ location: target, radius: 150 }, function (data, status) {
      ...
      svService.getPanorama({ location: target, radius: 500 }, function (data2, status2) {
        if (status2 === 'OK' && data2 && data2.location) {
          showSvLoading(st('txtSvNearby', '載入附近街景…'), st('txtSvNearbySub', '此座標附近無精確街景，已改用最接近位置'));
          applyPano(data2.location.latLng);
          return;
        }
        showSvLoading(st('txtSvNone', '此位置暫無 Google 街景'), st('txtSvNoneSub', '偏遠郊野／離島可能未有覆蓋。仍可答題；或於後台改用有街景的座標。'), { hideBar: true });
  ...
  window.startQuiz = function () {
    ...
    if (!loc.quizzes || !loc.quizzes.length) { alert(st('txtNoQuiz', '此站未設定問題')); return; }
    quizIndex = 0;
    quizCorrectCount = 0;
    showQuizQ();
    ...
    opts.querySelectorAll('button').forEach(function (btn) {
      btn.onclick = function () {
        ...
        if (correct) {
          ...
          fb.innerHTML = '<span class="text-emerald-400">' + st('txtCorrect', '答對了！') + '</span>';
          quizCorrectCount++;
        } else {
          btn.classList.add('bg-red-700');
          if (isOneShot()) {
            fb.innerHTML = '<span class="text-red-400">' + st('txtWrong', '答錯了') + '</span>';
          } else {
            fb.innerHTML = '<span class="text-red-400">' + st('txtWrongRetry', '答錯了，再試一次') + '</span>';
            setTimeout(function () {
  ...
  function finishLocationQuiz(loc) {
    ...
    if (allCorrect) {
      ...
      if (fb) fb.innerHTML = '<span class="text-emerald-400">' + st('txtAllCorrect', '全部答對！印章已收集！') + '</span>';
    } else {
      collected[current] = false;
      if (fb) fb.innerHTML = '<span class="text-amber-300">' + st('txtStationPartial', '本站完成（答對 {c} / {t}），未獲印章').replace('{c}', String(quizCorrectCount)).replace('{t}', String(loc.quizzes.length)) + '</span>';
    }
    rebuildPostcard();
    rebuildDots();
    updateActionButtons();
